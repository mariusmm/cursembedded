 ...
 * @version V1.0
 * @date    05-March-2019
 * @brief   vTaskDelayUntil FreeRTOS example with shorter times to
 * 			check with oscilloscope (PD7 is at EXP-15)
...
static void TaskLedToggle(void *pParameter) {
	...
	for (;;) {
		...
		vTaskDelayUntil(&previous_tick_time, pdMS_TO_TICKS(100));
	}
}
